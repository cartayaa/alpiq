...
function setFieldValue(field, value) { if ((formContext.getControl(field) != null) && (value != null)) { formContext.getAttribute(field).setValue(value); } }
...
function ContratoOnLoad(executionContext) {
    ...
    if (formContext.getAttribute("atos_cuentabancariaapropia").getValue() == null) {
        ...
        /* Comentado por Aleix Ruiz dÃ­a 30/11 - No se utiliza en producciÃ³n
 ...
 * 
 * Se ejecuta en la llamada para cargar la instalaciÃ³n segun instalacionid.
 ...
 * 
 * Se ejecuta en la llamada para cargar la informaciÃ³n del contrato desde la oferta
...
function successCallbackOferta(result) {                                                    /*223231*/
    ...
    if (result != null && result.entities != null && result.entities.length > 0) {
        ...
        if (oferta.atos_precisiondecimalesoferta != null) {
            ...
            formContext.getAttribute("atos_precisiondecimalesoferta").setSubmitMode("always");
        }
        ...
        // CondiciÃ³n de Pago. Tratamiento de Guid desde Fecth
        ...
        if (oferta.atos_commodity != null && oferta.atos_commodity == GAS) {
            ...
            //Fecha asignaciÃ³n agente facturaciÃ³n
        ...
        //Tipo GarantÃ­a solicitada al cliente
        ...
        //Fecha solicitud GarantÃ­a cliente
        ...
        //Fecha realizaciÃ³n Aval definitivo
        ...
        //Precio incluye impuesto elÃ©ctrico
...
function borrarMandatoSepa() {
    ...
    */
    if (formContext.getAttribute("atos_mandatosepa") != null) {
...
function WarningEMS(executionContext) {
    ...
    if (IsEmptyId()) {                                      /*223231*/
        ...
        {
            formContext.setFormNotification("La Ãºltima ejecuciÃ³n del WS de Contrato con EMS ha finalizado con errores", "WARNING", "1");
        }
        if (formContext.getAttribute("atos_interfazprecioscontratoems").getValue() == 300000000) // KO EMS
        {
            formContext.setFormNotification("La Ãºltima ejecuciÃ³n del WS de Precios Contrato con EMS ha finalizado con errores", "WARNING", "2");
        }
 ...
 * @param {executionContext} Contexto de ejecucion del formulario
 * Se ejecuta al guardar los datos. Valida las fechas y la cuenta bancaria (si es domiciliaciÃ³n bancaria)
 */
function Contrato_OnSave(executionContext) {
    ...
    if (formContext.getAttribute("atos_fechainiciogarantia").getIsDirty() == true ||
        ...
        if (compruebaFechas(executionContext, "atos_fechainiciogarantia", "atos_fechafingarantia", "La fecha de inicio de garantÃ­a debe ser menor o igual que la fecha de fin de garantÃ­a.", "7") != "")
 ...
 * 
 * Si el tipo de garantÃ­a no es Sin Garantia habilita los campos de garantÃ­a
 ...
 * @param {executionContext} Contexto de ejecucion del formulario
 * Habilita o no los datos de garantÃ­a en funciÃ³n del tipo de garantÃ­a
 */
 ...
 * Function from FechaInicioContrato event
 * @param {executionContext} Contexto de ejecucion del formulario
 * Inicializa (si estÃ¡ vacÃ­a), la fecha inicio vigencia de precios con el valor de la fecha de inicio del contrato
 */
