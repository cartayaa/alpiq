...
namespace CierreOfertaCoefApun
{
    ...
    public class CierreOferta : IPlugin
    {
        ...
        */
        ...
        private void writelog(String texto, bool _log = false)
        {
            if (_log == true)
                TracingService.Trace(texto);
            // Log.writelog(texto, _log);
            /*if (_traza)
                TracingService.Trace(texto);
            if (_log == true)
        ...
        {
            this.PluginExecutionContext = (IPluginExecutionContext)serviceProvider.GetService(typeof(IPluginExecutionContext)); // Obtain the execution context service from the service provider.
            this.TracingService = (ITracingService)serviceProvider.GetService(typeof(ITracingService)); // Obtain the tracing service from the service provider.
            IOrganizationServiceFactory factory = (IOrganizationServiceFactory)serviceProvider.GetService(typeof(IOrganizationServiceFactory));            // Obtain the Organization Service factory service from the service provider
            this.OrganizationService = factory.CreateOrganizationService(this.PluginExecutionContext.UserId); // Use the factory to generate the Organization Service.
