    ...
    class Program
    {
        ...
        private static bool? tratarFichero(string pFicheroEntrada, string pNombreFichero)
        {
            ...
            {              
                ...
                // comporbamos que no este duplicada la peticion
                if (!proceso.ValidarPeticionRepetida(_servicioCRM))
                {
                    _logFichero.Add(String.Format("Error en la lectura del fichero:{0};{1}", pNombreFichero, "la petición ya se encuentra cargada en el sistena."));
                    salida = false;
                    return salida;
                }
                ...
                {
                    int numMensajes = proceso.ObtenerNumeroMensajes();
                    for (proceso.NumeroMensaje = 0; proceso.NumeroMensaje < numMensajes; proceso.NumeroMensaje++)
                    {
                        Entity entidadCrm = EntidadCrmFactoria.CrearEntidad(proceso.TipoEntidad);
                        ProcesoResultado resultado = proceso.EjecutarPaso(entidadCrm);
                        if (resultado.Exito)
                        {
                            salida = true;
                            proceso.AdjuntarXml(resultado.CampoID, entidadCrm.LogicalName, _dirFichsNoUri, pNombreFichero);
                        }
                        else
                            _logFichero.AddRange(resultado.Errores);
                    }
                }
                else
                    _logFichero.Add(String.Format("Error en la lectura del fichero:{0};{1}", pNombreFichero, "El agente, proceso y paso son datos obligatorios"));
            }
            catch (Exception ex)
            {
