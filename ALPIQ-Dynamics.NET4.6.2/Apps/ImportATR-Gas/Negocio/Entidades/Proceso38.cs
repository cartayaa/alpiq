...
{
    public class Proceso38 : ProcesoBase
    {
        public Proceso38(string nombreFichero, List<String> log, XmlDocument xDoc, Operaciones operaciones, TipoEntidad tipoEntidad)
        ...
        public override int ObtenerNumeroMensajes()
        {
            try
            {
                string nombreElemento = (_paso).ToLower();
        ...
        {
            _operaciones.ProcesarPasoA238(_nombreFichero, _xDoc, entidadCRM, _rechazo, _agente, NumeroMensaje);
        }

        private void procesarPasoA25(Entity entidadCRM)
        {
            _operaciones.ProcesarPasoA2538(_nombreFichero, _xDoc, entidadCRM, _rechazo, _agente, NumeroMensaje);
        }

        private void procesarPasoA3(Entity entidadCRM)
        {
            _operaciones.ProcesarPasoA338(_nombreFichero, _xDoc, entidadCRM, _rechazo, _agente, NumeroMensaje);
        }

        private void procesarPasoA4(Entity entidadCRM)
        {
            _operaciones.ProcesarPasoA438(_nombreFichero, _xDoc, entidadCRM, _rechazo, _agente, NumeroMensaje);
        }
    }
}
